...
rm(list = ls(all = TRUE))
options(warn = -1)
setwd("~/Dropbox/PlathLab/Analyses/Justin_Langerman/Timecourse_analysis/other/factor-binding-analysis-brg1/"); getwd()
...
################################################
## BINDING DATA FOR BRG1
dir <- "~/Dropbox/PlathLab/Analyses/Justin_Langerman/Timecourse_analysis/data/kostas_peaks/other_factors/"
beds <- list.files(dir, pattern = "*Brg1.peaks.bed", full.names = TRUE, recursive = TRUE)
binding <- lapply(beds, bed_to_granges)
names(binding) <- basename(beds)
saveRDS(binding, file.path(getwd(), "BINDING-PATTERNS.rds"), compress = TRUE)
...
programs <- readRDS("~/Dropbox/PlathLab/Analyses/Justin_Langerman/Timecourse_analysis/data/metanetwork/2019.02.08.Drop10.All.Program.List.Cleanedx4.rds")
...
programs_set <- split(programs, f = programs$Set)
programs_run <- split(programs, f = programs$Run)
...
genes_bound_for_each_pattern_bg <- data.frame(tmp = sapply(seq_along(binding), function(y){
  binding_pattern <- tools::file_path_sans_ext(tools::file_path_sans_ext(names(binding)[y]))
...
# programs <- readRDS("~/Dropbox/PlathLab/Analyses/Justin_Langerman/Timecourse_analysis/data/metanetwork/2019.01.23.Drop10.All.Program.List.Cleanedx2.v2.rds")
# order_of_progs <- unique(programs[, c("Order", "Set")])
# order_of_progs <- order_of_progs[with(order_of_progs, order(Order)),]
# order_of_progs <- order_of_progs[c(2,4,1,3, seq(5, nrow(order_of_progs), 1)),]

order_of_progs <- gtools::mixedsort(names(programs))

pheatmap(enrichment[c(3, 1, 4, 2), order_of_progs], 
         cluster_cols = FALSE,
         cluster_rows = FALSE,
         display_numbers = TRUE,
         annotation_col = annot, 
