...
rm(list = ls(all = TRUE))
options(warn = -1)
setwd("~/Dropbox/PlathLab/Analyses/Justin_Langerman/Timecourse_analysis/other/overlap-genes-with-atac-and-state-w-bg/")
...
write.table(atac_bed, file=paste(getwd(), "ALL-ATAC-PEAKS.bed", sep = "/"), quote=F, sep="\t", row.names=F, col.names=F)
...
atac <- state_overlap(peak = atac, states = enhancers, extend_upstream = 0, extend_downstream = 0)



### WRITE OUT BG
bg <- atac
# ggplot(data.frame(w = width(bg)), aes(w)) + geom_density()
x = as.data.frame(bg)
...
write.table(bed, file=paste(getwd(), "BG-EXTENDED-AND-MERGED.bed", sep = "/"), quote=F, sep="\t", row.names=F, col.names=F)
...
programs <- droplevels.data.frame(subset(programs, Set %in% c("CDH1_1", "CDH1_8", "TC1_4.02"))) # subset)
...
homer <- lapply(seq_along(merged_overlaps), function(o) {
  ...
  run_homer(file = f, out = dir, bg = paste(getwd(), "BG-EXTENDED-AND-MERGED.bed", sep = "/"), dry = FALSE)
